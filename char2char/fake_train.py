...
if __name__ == "__main__":
    ...
    parser.add_argument("--char_vocab", default='', type=str,
                        help="Path to file storing char vocabulary. If no provided, is automatically computed from data.")
    ...
    experiment_name += '_layers{}_dim{}_embedding{}_lr{}_charvocab{}'.format(args.num_layers, args.rnn_cell_dim,
                                                                             args.embedding,
                                                                             args.learning_rate, args.char_vocab)
    ...
    char_vocab = None
    if args.char_vocab != '':
        char_vocab = utils.load_vocabulary(args.char_vocab)

    dataset = parallelsentences_chars.ParalelSentencesDataset(args.batch_size, args.max_chars, input_sentences,
                                                              target_sentences,
                                                              args.train_perc, args.validation_perc, args.test_perc,
                                                              char_vocab, args.num_top_chars)

    if 'dev_inputs' in dataset_files:
        print('Loading validation data')
        eval_set_input_sentences, eval_set_target_sentences = [], []
        with io.open(dataset_files['dev_inputs'], 'r', encoding='utf8') as reader:
            eval_set_input_sentences = reader.read().splitlines()

        with io.open(dataset_files['dev_targets'], 'r', encoding='utf8') as reader:
            eval_set_target_sentences = reader.read().splitlines()

        dataset.add_validation_set(eval_set_input_sentences, eval_set_target_sentences)
    ...
    network.restore('/home/naplava/troja/diacritization_matching/char2char/save/ces_pokus_layers2_dim300_embedding300_lr0.0003_charvocab/2017-10-26_140754')
    ...
    with io.open('pom.txt', 'w') as out:
        ...
        for eval_set_name in evaluation_sets.keys():
            ...
            for eval_set_input_sentence, eval_set_sentence_len, eval_set_target_sentence in zip(
                ...
                out.write(u'eval_set_input_sentence: {}\n'.format(eval_set_input_sentence))
                out.write(u'eval_set_sentence_len: {}\n'.format(eval_set_sentence_len))
                out.write(u'eval_set_target_sentence: {}\n'.format(eval_set_target_sentence))
                # if i == 0:
                #     print('000')
                #     print(eval_set_input_sentence)
                #     print(eval_set_sentence_len)
                #     print(eval_set_target_sentence)
                ...
                out.write(u'eval_partial_set_result: {}\n'.format(eval_partial_set_result))
                # print('eval_partial_set_result', eval_partial_set_result[:eval_set_sentence_len[0]])
            ...
            # print('eval_set_input_sentences', eval_set_input_sentences[0][0], ''.join([value_to_key(x, char_vocab) for x in eval_set_input_sentences[0][0]]))
            # print('eval_set_target_sentence', eval_set_target_sentences[0][0] , ''.join([value_to_key(x, char_vocab) for x in eval_set_target_sentences[0][0]]))
            ...
            for metric_name, metric_fn in evaluation_metrics.items():
                ...
                result = metric_fn(predictions, lengths, targets, dataset.char_vocab)
